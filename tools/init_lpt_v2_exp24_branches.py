"""初始化 LPT v2 第 24 项 RetNetAssist Q/K adapter 实验分支。"""
...
DEFAULT_OUTPUT_ROOT = PROJECT_ROOT / "artifacts" / "lpt_v2" / "experiments_exp24"

BRANCH_DEFINITIONS = {
    "base_continued": {
        "description": "同等预算继续训练对照；text_pretrain 基座必须已经是 Q-only RetNetAssist。",
        "overrides": {},
        "init_strategy": "完全沿用 base checkpoint 配置和权重，同时作为 Q-only RetNetAssist baseline。",
    },
    "exp_24_qk_adapter": {
        "description": "RetNetAssist 从 Q-only 扩展为 Q/K adapter，用于评估 K 注入收益和成本。",
        "overrides": {
            "retnet_assist_mode": "qk_adapter",
            "retnet_adapter_target": ("q", "k"),
            "retnet_k_adapter_enabled": True,
        },
        "init_strategy": "加载 base 可匹配权重；新增每层 K adapter down/up projection 与 alpha_k，按默认小 scale 初始化。",
    },
}


def build_parser():
    parser = argparse.ArgumentParser(description="初始化 LPT v2 第 24 项实验分支 checkpoint。")
    ...
    parser.add_argument(
        "--branches",
        default="base_continued,exp_24_qk_adapter",
        help="逗号分隔分支名。",
    )
    return parser
...
def _count_retnet_enabled_layers(config):
    ...
    if policy.startswith("every_") and policy.endswith("_layers"):
        ...
        if interval_text.isdigit():
            ...
            return (int(config.num_layers) + interval - 1) // interval
    return 0


def _assert_base_contract(base_config):
    """第 24 项只能从 Q-only RetNetAssist 且未启用 xLSTM 的共同基座开始。"""
    ...
    if base_config.retnet_k_adapter_enabled:
        raise ValueError("base checkpoint 已启用 retnet_k_adapter_enabled，不能作为第 24 项共同基座。")
    if base_config.xlstm_memory_enabled:
        raise ValueError("base checkpoint 已启用 xLSTMAssist，会污染第 24 项 K adapter 单项归因。")
...
def main(argv=None):
    ...
    for branch_name in branches:
        ...
        report = {
            "experiment": "lpt_v2_exp24_qk_adapter",
            "branch": branch_name,
            ...
            "retnet_assist_mode": branch_config.retnet_assist_mode,
            "retnet_adapter_target": list(branch_config.retnet_adapter_target),
            "retnet_k_adapter_enabled": bool(branch_config.retnet_k_adapter_enabled),
            "retnet_enabled_layer_count": _count_retnet_enabled_layers(branch_config),
        ...
        print(
            "branch_initialized="
            f"{branch_name} retnet_target={report['retnet_adapter_target']} checkpoint={checkpoint_path}"
        )

    _write_json(output_root / "exp24_init_summary.json", {"branches": reports})
    print(f"summary={output_root / 'exp24_init_summary.json'}")
    return 0
